...
function Testimonials({ testimonials, animationConfig }) {
  ...
  return (
    <>
      {/* Mobile */}
      <Section className='mx-auto  lg:hidden block' {...animationConfig}>
        <div className='flex flex-row gap-10 justify-center items-center '>
          <img src={roundIcon} className='w-[6rem]' alt="" />
          <p className='opacity-90 -ms-8 text-2xl text-white'>Testimonials</p>
        </div>
        <Carousel className="flex w-auto sm:w-[60%] mx-auto rounded-xl text-white mt-10">
        {testimonials.map((list, index) => (
            <div key={index} className='flex flex-col h-full'>
              <Card className='h-full bg-[#252630] p-8 pb-0 text-white'>
                <p className='text-2xl text-center '><span className='text-red-500'>What they say</span> <span className=' bg-gradient-to-r from-white to-gray-600  text-transparent bg-clip-text'>about us?</span></p>
              ...
              <div className='p-5 h-[15rem] w-[82%] rounded-b-[1.1rem] mx-auto bg-gray-900'>
              <Text align='' text={`${truncateDescription(list?.description)}`} />
      ...
      {/* Laptop */}
      <Section className='h-[90vh] mx-auto hidden lg:block' {...animationConfig}>
