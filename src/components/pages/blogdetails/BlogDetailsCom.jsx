...
function BlogDetailsCom({ animationConfig, data, suggestions }) {
      ...
      <Details className='flex flex-col lg:flex-row items-start gap-6'>
        <div className='w-full flex flex-col-reverse lg:flex-row gap-6'>
          ...
          {data?.body ? (<TextContent className='bg-gray-900 bg-opacity-50 w-full rounded-[1.1rem] rounded-t-none lg:rounded-t-[1.1rem] p-6' >
            ...
            <TextContent className='bg-gray-900 bg-opacity-50 w-full rounded-[1.1rem] rounded-t-none lg:rounded-t-[1.1rem] flex justify-center items-center p-6'>
        ...
        <Suggested className='w-full lg:w-5/12 h-auto bg-gray-900 bg-opacity-50 rounded-[1.1rem] px-6'>
          ...
          <div className='flex flex-col gap-5 lg:gap-0 my-2 lg:my-5'>
            {suggestions[0] &&
              <>
                <div className='flex flex-row gap-2 lg:gap-5 items-center cursor-pointer' onClick={() => navigate(`/blog/${suggestions[0].slug}`)}>
                  <img src={suggestions[0]?.image} alt={suggestions[0]?.image_alt} className='h-[5rem] w-[7rem]' />
            ...
            {suggestions[1] &&
              <>
                <div className='flex flex-row gap-2 lg:gap-5 items-center cursor-pointer' onClick={() => navigate(`/blog/${suggestions[1].slug}`)}>
                  <img src={suggestions[1]?.image} alt={suggestions[1]?.image_alt} className='h-[5rem] w-[7rem]' />
                  <Typography variant="p" className="text-white text-[.9rem] text-opacity-90">
                    {suggestions[1]?.title}
                  </Typography>
                </div>
            ...
            {suggestions[2] && <div className='flex flex-row gap-2 lg:gap-5 items-center cursor-pointer' onClick={() => navigate(`/blog/${suggestions[2].slug}`)}>
              <img src={suggestions[2]?.image} alt={suggestions[2]?.image_alt} className='h-[5rem] w-[7rem]' />
              <Typography variant="p" className="text-white text-[.9rem] text-opacity-90">
                {suggestions[2]?.title}
              </Typography>
            </div>}
